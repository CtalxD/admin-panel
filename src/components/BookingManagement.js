...
const BookingManagement = () => {
  ...
  const handleDeleteBooking = async (id) => {
    if (window.confirm('Are you sure you want to delete this booking?')) {
      ...
      try {
        ...
        setError({ type: 'success', message: 'Booking deleted successfully' });
        fetchBookings();
      } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (missingFields.length > 0) {
      setError({ type: 'error', message: `Missing required fields: ${missingFields.join(', ')}` });
      setIsLoading(false);
      return;
    }
    
    if (currentBooking.passengerNames.length === 0) {
      setError({ type: 'error', message: 'At least one passenger is required' });
      setIsLoading(false);
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.message || (currentBooking.id ? 'Failed to update booking' : 'Failed to create booking'));
      }
      
      setError({ type: 'success', message: `Booking ${currentBooking.id ? 'updated' : 'created'} successfully` });
      setShowModal(false);
      fetchBookings();
    } catch (error) {
  ...
  const updateBookingStatus = async (id, newStatus) => {
    ...
    try {
      ...
      setError({ type: 'success', message: `Booking status updated to ${newStatus}` });
      fetchBookings();
    } catch (error) {
    ...
    <div className="booking-management-container">
      <div className="header-section">
        ...
        <div className="controls">
          ...
          <button onClick={fetchBookings} className="refresh-button" disabled={isLoading}>
            <svg className="refresh-icon" viewBox="0 0 24 24">
              <path d="M17.65 6.35C16.2 4.9 14.21 4 12 4c-4.42 0-7.99 3.58-7.99 8s3.57 8 7.99 8c3.73 0 6.84-2.55 7.73-6h-2.08c-.82 2.33-3.04 4-5.65 4-3.31 0-6-2.69-6-6s2.69-6 6-6c1.66 0 3.14.69 4.22 1.78L13 11h7V4l-2.35 2.35z"/>
            </svg>
            {isLoading ? 'Refreshing...' : 'Refresh'}
          </button>
          <button onClick={handleAddBooking} className="add-button" disabled={isLoading}>
      ...
      {error && (
        <div className={`alert-message ${error.type}`}>
          {error.message}
          ...
          <div className="modal-content">
            ...
            <form onSubmit={handleSubmit}>
              <div className="modal-body">
                {error && (
                  <div className={`modal-message ${error.type}`}>
                    <svg viewBox="0 0 24 24">
                ...
                >
                  {isLoading ? (
                    <>
                      ...
                      {currentBooking.id ? 'Saving...' : 'Creating...'}
                    </>
                  ) : (
